...
import { MorpheusPillChoice } from '@/components/brand/MorpheusPillChoice';
...
function LoginFormContent() {
  ...
  // Mode: 'choice' (Morpheus 2-Pill Challenge) | 'login' (Operator Auth Form)
  const [viewMode, setViewMode] = useState<'choice' | 'login'>('choice');
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center p-4 sm:p-6 overflow-hidden bg-[#02060C] select-none">
      ...
      {/* 🕶️ INTERACTIVE VIEW SWITCHER (Choice vs Login) */}
      <AnimatePresence mode="wait">
        {viewMode === 'choice' ? (
          <motion.div
            key="morpheus-choice-screen"
          ...
          >
            <MorpheusPillChoice onSelectRed={() => setViewMode('login')} />
          </motion.div>
        ) : (
          /* Main Glassmorphic Terminal Login Card */
          <motion.div
            key="matrix-login-card"
          ...
          >
            <div className="relative p-7 sm:p-9 rounded-3xl bg-[#070D18]/90 backdrop-blur-2xl border border-emerald-500/30 shadow-[0_0_60px_rgba(16,185,129,0.18)] space-y-6 overflow-hidden">
              ...
              {/* Clean Cyberpunk Footer & Re-enter Choice Button */}
