...
export function MetricCards({ stats, isLoading }: MetricCardsProps) {
  ...
  const cards = [
    ...
    {
      title: 'Orders Placed',
      displayValue: stats.financials?.totalOrders ?? 0,
      subtext: `${stats.channels?.frequentBuyerCount ?? 0} repeat buyers`,
      icon: ShoppingBag,
      iconClass: 'text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-500/10',
    },
    {
      title: 'VIP Clients',
      displayValue: stats.channels?.vipCount ?? 0,
      subtext: `${stats.channels?.vipRatio ?? 0}% of customer base`,
      icon: Star,
      iconClass: 'text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10',
    },
    {
      title: 'WhatsApp Active',
      displayValue: stats.channels?.whatsappCount ?? 0,
      subtext: `${stats.channels?.whatsappRatio ?? 0}% direct reach rate`,
      icon: MessageCircle,
