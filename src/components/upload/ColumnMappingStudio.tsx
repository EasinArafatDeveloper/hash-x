...
export const TARGET_SYSTEM_FIELDS = [
  ...
  { value: 'matched_district_filters', label: 'Matched District (matched_district_filters)', icon: Compass, color: 'text-blue-500 dark:text-blue-400' },
  { value: 'matched_city_filters', label: 'Matched City (matched_city_filters)', icon: Building2, color: 'text-indigo-500 dark:text-indigo-400' },
  { value: 'matched_area_filters', label: 'Matched Area (matched_area_filters)', icon: MapPin, color: 'text-teal-500 dark:text-teal-400' },
  { value: 'matched_block_road_filters', label: 'Matched Block / Road (matched_block_road_filters)', icon: Navigation, color: 'text-amber-500 dark:text-amber-400' },
  ...
  { value: 'lifetime_primary_category', label: 'Primary Category (lifetime_primary_category)', icon: Tag, color: 'text-orange-500 dark:text-orange-400' },
...
export function ColumnMappingStudio({
  ...
  return (
    <div className="rounded-xl bg-white dark:bg-[#111113] border border-gray-200 dark:border-white/10 shadow-card overflow-hidden transition-all duration-200">
      {/* Collapsible Header */}
      <div
        onClick={onToggleOpen}
        className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-gray-50 dark:bg-white/[0.03] cursor-pointer select-none hover:bg-gray-100 dark:hover:bg-white/[0.05] transition-colors"
      >
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-brand-50 dark:bg-brand-500/10 text-brand-600 dark:text-brand-400 border border-brand-200 dark:border-brand-900/60 flex items-center justify-center shrink-0">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="px-2.5 py-0.5 rounded-full bg-brand-50 dark:bg-brand-500/10 text-brand-700 dark:text-brand-300 border border-brand-200 dark:border-brand-900/60 font-bold text-[10px] uppercase tracking-wider">
                {isOpen ? 'Customizing' : 'Active'}
              </span>
              {skippedColumnsCount > 0 && (
                <span className="px-2 py-0.5 rounded-full bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 border border-rose-200 dark:border-rose-900/60 font-bold text-[10px]">
        ...
        <div className="flex items-center gap-2 self-start sm:self-center">
          {phoneMappedColumn ? (
            <span className="px-2.5 py-1 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-900/60 text-[11px] font-bold flex items-center gap-1.5">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
              <span>Mobile No: &ldquo;{phoneMappedColumn}&rdquo;</span>
            </span>
          ) : (
            <span className="px-2.5 py-1 rounded-lg bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-900/60 text-[11px] font-bold flex items-center gap-1.5">
          ...
          <button
            type="button"
            className="px-3 py-1.5 rounded-lg bg-white dark:bg-white/5 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-white/10 text-xs font-bold hover:bg-gray-50 dark:hover:bg-white/10 transition-colors"
          >
      ...
      {isOpen && (
        <div className="p-4 sm:p-6 space-y-4 border-t border-gray-100 dark:border-white/10 bg-white dark:bg-[#111113] animate-in fade-in duration-200">
          {/* Top Quick Actions Toolbar & Smart Upsert Notice */}
          <div className="p-3.5 rounded-xl bg-emerald-50/60 dark:bg-emerald-500/5 border border-emerald-200 dark:border-emerald-900/50 text-xs flex items-start gap-3">
            <div className="p-1.5 rounded-lg bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 shrink-0 mt-0.5">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3.5 rounded-xl bg-gray-50 dark:bg-white/[0.03] border border-gray-200 dark:border-white/10 text-xs">
            ...
            <div className="flex items-center gap-2 flex-wrap self-start sm:self-center">
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg bg-violet-600 hover:bg-violet-700 text-white text-[11px] font-bold flex items-center gap-1.5 transition-colors cursor-pointer active:scale-95 disabled:opacity-50"
              >
                {isAiMappingLoading ? (
              ...
              <button
                ...
                className="px-2.5 py-1.5 rounded-lg bg-white dark:bg-white/5 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-900/60 hover:bg-rose-50 dark:hover:bg-rose-500/10 text-[11px] font-bold flex items-center gap-1.5 transition-colors cursor-pointer active:scale-95"
              >
              ...
              <button
                ...
                className="px-2.5 py-1.5 rounded-lg bg-white dark:bg-white/5 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-white/10 text-[11px] font-bold flex items-center gap-1.5 transition-colors cursor-pointer active:scale-95"
              >
              ...
              <button
                ...
                className="px-2.5 py-1.5 rounded-lg bg-white dark:bg-white/5 text-brand-600 dark:text-brand-400 border border-brand-200 dark:border-brand-900/60 hover:bg-brand-50 dark:hover:bg-brand-500/10 text-[11px] font-bold flex items-center gap-1.5 transition-colors cursor-pointer active:scale-95"
              >
          ...
          <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-white/10">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-gray-50 dark:bg-white/[0.03] text-gray-600 dark:text-gray-300 uppercase tracking-wider text-[10px] font-bold border-b border-gray-200 dark:border-white/10">
                  ...
                  return (
                    <tr
                      ...
                      className={`transition-colors ${
                        isSkipped
                          ? 'bg-rose-50/40 dark:bg-rose-500/[0.03] opacity-70 hover:opacity-90'
                          : isPhoneKey
                          ? 'bg-emerald-50/40 dark:bg-emerald-500/[0.03] hover:bg-emerald-50/70'
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="truncate">
                            ...
                            {isSkipped ? (
                              ...
                              <span
                                className="inline-flex items-center gap-1 text-[10px] font-bold text-violet-600 dark:text-violet-400"
                            ...
                            samples.map((sVal, sIdx) => (
                              <span
                                ...
                                className={`px-2 py-0.5 rounded-md font-mono text-[11px] truncate max-w-[130px] border ${
                                  isSkipped
                                    ? 'bg-gray-50 dark:bg-white/[0.02] text-gray-400 border-gray-200/50 dark:border-white/5'
                                    : 'bg-gray-100 dark:bg-white/5 text-gray-700 dark:text-gray-300 border-gray-200/80 dark:border-white/10'
                                }`}
                                title={String(sVal)}
                              >
                                {String(sVal)}
                        ...
                        <div className="flex items-center gap-2">
                          <div className="relative flex-1">
                            <select
                              ...
                              className={`w-full py-2 pl-3 pr-8 rounded-lg text-xs font-bold bg-white dark:bg-white/5 border transition-all focus:outline-none focus:ring-2 focus:ring-brand-500 cursor-pointer ${
                                currentField === 'phone'
                                  ? 'border-emerald-300 dark:border-emerald-900/60 text-emerald-700 dark:text-emerald-300 bg-emerald-50/60 dark:bg-emerald-500/[0.05]'
                                  : currentField === 'skip'
                                  ? 'border-rose-300 dark:border-rose-900/60 text-rose-600 dark:text-rose-400 bg-rose-50/50 dark:bg-rose-500/[0.05]'
                                  : 'border-brand-300 dark:border-brand-900/60 text-brand-700 dark:text-brand-300 bg-brand-50/40 dark:bg-brand-500/[0.05]'
                              }`}
                            >
                              {TARGET_SYSTEM_FIELDS.map((target) => (
                                <option
                                  key={target.value}
                                  value={target.value}
                                  className="text-gray-900 dark:text-white bg-white dark:bg-[#111113] font-medium py-1"
                                >
                                  {target.label}
                                </option>
                              ))}
                          ...
                          {isSkipped ? (
                            <button
                              ...
                              className="px-3 py-2 rounded-lg bg-rose-50 hover:bg-rose-100 dark:bg-rose-500/10 dark:hover:bg-rose-500/20 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-900/60 text-xs font-bold flex items-center gap-1.5 transition-colors shrink-0 cursor-pointer active:scale-95"
                            >
                            ...
                            <button
                              ...
                              className="px-3 py-2 rounded-lg bg-gray-100 hover:bg-rose-50 dark:bg-white/5 dark:hover:bg-rose-500/10 text-gray-600 hover:text-rose-600 dark:text-gray-300 dark:hover:text-rose-300 border border-gray-200 hover:border-rose-300 dark:border-white/10 dark:hover:border-rose-900/60 text-xs font-semibold flex items-center gap-1.5 transition-colors shrink-0 cursor-pointer active:scale-95 group"
                            >
          ...
          <div className="p-3.5 rounded-xl bg-gray-50 dark:bg-white/[0.03] border border-gray-200 dark:border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs text-gray-500 dark:text-gray-400">
