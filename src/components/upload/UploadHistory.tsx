...
export function UploadHistory({ refreshKey = 0, onDatasetDeleted }: UploadHistoryProps) {
  ...
  const getFileIcon = (filename: string) => {
    ...
    if (isExcel) {
      return (
        <div className="p-2.5 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-900/60 shrink-0">
          <FileSpreadsheet className="w-5 h-5" />
        </div>
      );
    }
    return (
      <div className="p-2.5 rounded-lg bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-900/60 shrink-0">
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold text-gray-900 dark:text-white flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded-full bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-300 text-[11px] font-semibold">
        ...
        <button
          ...
          className="p-2 rounded-lg text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-100 hover:bg-gray-100 dark:hover:bg-white/5 transition-colors disabled:opacity-50"
      ...
      {isLoading && datasets.length === 0 && (
        <div className="p-6 rounded-xl bg-white dark:bg-[#111113] border border-gray-200 dark:border-white/10 space-y-3">
          <div className="h-5 bg-gray-100 dark:bg-white/5 rounded-lg w-1/3 animate-pulse" />
          <div className="h-16 bg-gray-50 dark:bg-white/[0.03] rounded-lg animate-pulse" />
          <div className="h-16 bg-gray-50 dark:bg-white/[0.03] rounded-lg animate-pulse" />
        </div>
      )}
      ...
      {/* Empty State */}
      {!isLoading && datasets.length === 0 && (
        <div className="p-8 text-center rounded-xl bg-white dark:bg-[#111113] border border-gray-200 dark:border-white/10 shadow-card space-y-3">
      ...
      {/* Datasets List */}
      {datasets.length > 0 && (
        <div className="bg-white dark:bg-[#111113] border border-gray-200 dark:border-white/10 rounded-xl shadow-card divide-y divide-gray-100 dark:divide-white/5 overflow-hidden">
          {datasets.map((item) => {
            ...
            return (
              <div
                key={item._id}
                className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-gray-50 dark:hover:bg-white/[0.03] transition-colors group"
              >
                {/* Left: Icon & File Meta */}
                ...
                <div className="flex items-center space-x-2 shrink-0 self-end sm:self-center">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-brand-50 hover:bg-brand-100 dark:bg-brand-500/10 dark:hover:bg-brand-500/20 text-brand-700 dark:text-brand-300 border border-brand-200 dark:border-brand-900/60 text-xs font-semibold transition-colors active:scale-95 cursor-pointer"
                  ...
                  <Link
                    href="/data/explorer"
                    className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-gray-100 dark:bg-white/5 text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-white/10 text-xs font-semibold transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-rose-50 hover:bg-rose-100 dark:bg-rose-500/10 dark:hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-900/60 text-xs font-semibold transition-colors active:scale-95 cursor-pointer"
